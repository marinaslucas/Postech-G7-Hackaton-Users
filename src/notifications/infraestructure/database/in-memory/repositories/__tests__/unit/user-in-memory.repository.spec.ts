...
import { UserInMemoryRepository } from '../../notification-in-memory.repository';
...
describe('UserInMemoryRepository unit tests', () => {
  ...
  it('Should throw error when not found - findByEmail method', async () => {
    await expect(sut.findByEmail('a@a.com')).rejects.toThrow(
      new NotFoundError('User not found for email provided a@a.com')
    );
  });
  ...
  it('Should find a entity by email - emailExists method', async () => {
    const entity = new UserEntity(userDataBuilder({}));
    await expect(sut.emailExists(entity.email)).resolves.not.toThrow();
    await sut.insert(entity);
    await expect(sut.emailExists(entity.email)).rejects.toThrow(
      new ConflictError('Email already exists')
    );
